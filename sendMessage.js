...
const userVipTickEl = document.getElementById("user-vip-tick");
const userRewardEmojiEl = document.getElementById("user-reward-emoji");
...
async function init() {
  ...
  try {
    ...
    const perks = userData.unlockedPerks || [];
    const customEmojis = Array.isArray(userData.customEmojis) ? userData.customEmojis.filter(Boolean).slice(0, 12) : [];
...
// ─── Playback: Keystroke recorder ────────────────────────────
// Records every input event as {value, t} where t is ms since first keystroke.
// This captures typing, deletions, pastes — the full editing journey.
let keystrokeLog = [];
let recordingStartTime = null;

function resetKeystrokeLog() {
  keystrokeLog = [];
...
messageTextEl?.addEventListener("input", () => {
  ...
  // Store the full textarea value at this moment + relative timestamp
  keystrokeLog.push({
    v: messageTextEl.value,
...
btnSend?.addEventListener("click", async () => {
  ...
  try {
    await setDoc(doc(db, "messages", deleteToken), {
      ...
      keystrokeLog: keystrokeLog.length > 1 ? keystrokeLog : []
    });
