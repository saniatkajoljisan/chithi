...
fontChoiceEls.forEach((option) => {
  option.addEventListener("change", () => {
    syncStyleFromFont(getSelectedFontKey());
  });
});
...
function syncStyleFromFont(fontKey) {
  const normalizedFont = normalizeFontKey(fontKey);
  const matchingBg = fontToBg[normalizedFont] || "paper";
  const bgInput = document.querySelector(`input[name="letter-bg"][value="${matchingBg}"]`);
  if (bgInput) bgInput.checked = true;
  applyComposeFont(normalizedFont);
  applyComposeBg(matchingBg);
}

function syncStyleFromBg(bgKey) {
  const normalizedBg = normalizeBgKey(bgKey);
  const matchingFont = bgToFont[normalizedBg] || "hand-caveat";
  const fontInput = document.querySelector(`input[name="letter-font"][value="${matchingFont}"]`);
  if (fontInput) fontInput.checked = true;
  applyComposeFont(matchingFont);
  applyComposeBg(normalizedBg);
}
