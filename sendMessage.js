...
function getUsernameFromPath() {
  ...
  console.log("URL parsing - pathname:", window.location.pathname);
  console.log("URL parsing - parts:", parts);
  console.log("URL parsing - last part:", last);

  if (reservedRoutes.has(last) || last.includes(".")) {
    console.log("Last part is reserved or contains dot, returning empty");
    return "";
  }
  const result = /^[a-z0-9_]{3,20}$/i.test(last) ? decodeURIComponent(last) : "";
  console.log("Username from path result:", result);
  return result;
}
...
const languageChoiceEls = document.querySelectorAll('input[name="letter-language"]');
...
const allowedFontKeys = new Set([
  ...
  "bn-siliguri", "bn-baloo", "bn-notoserif", "bn-sans"
]);
const allowedBgKeys = new Set([
  "paper", "rose", "mint", "sky",
  "bn-paper", "bn-rose", "bn-mint", "bn-sky"
]);
const styleToKeys = {
  ...
  "bn-1": { font: "bn-siliguri", bg: "bn-paper" },
  "bn-2": { font: "bn-baloo", bg: "bn-rose" },
  "bn-3": { font: "bn-notoserif", bg: "bn-mint" },
  "bn-4": { font: "bn-sans", bg: "bn-sky" }
};
const blockedWords = [
...
// ─── Init ───────────────────────────────────────────────────
async function init() {
  console.log("Initializing sendMessage.js for username:", username);
  ...
  try {
    ...
    console.log("Firebase connection successful, total users in DB:", testSnap.size);
  } catch (testErr) {
    console.error("Firebase connection test failed:", testErr);
  }

  if (!username) {
    console.log("No username found, showing not found");
    showNotFound();
    return;
  }

  try {
    console.log("Looking up user:", username.toLowerCase());
    // Look up the user by username
    const q = query(collection(db, "users"), where("username", "==", username.toLowerCase()));
    console.log("Query created, executing getDocs...");
    const snap = await getDocs(q);
    console.log("Query completed, snapshot size:", snap.size);
...
languageChoiceEls.forEach((option) => {
  option.addEventListener("change", () => {
...
function getSelectedLanguage() {
  const selected = document.querySelector('input[name="letter-language"]:checked');
  return selected?.value === "bn" ? "bn" : "en";
}

function applyLanguage(language) {
  const enStyleRow = document.querySelector(".style-choice-row.en");
  const bnStyleRow = document.querySelector(".style-choice-row.bn");

  if (enStyleRow && bnStyleRow) {
    enStyleRow.classList.toggle("hidden-language", language !== "en");
    bnStyleRow.classList.toggle("hidden-language", language !== "bn");
  }

  const currentStyle = getSelectedStyleKey();
  if (language === "bn" && !currentStyle.startsWith("bn-")) {
    const defaultBnOption = document.querySelector('input[name="letter-style"][value="bn-1"]');
    if (defaultBnOption) defaultBnOption.checked = true;
    applyStyle("bn-1");
  }
  if (language === "en" && !currentStyle.startsWith("en-")) {
    const defaultEnOption = document.querySelector('input[name="letter-style"][value="en-1"]');
    if (defaultEnOption) defaultEnOption.checked = true;
    applyStyle("en-1");
  }
}
...
function applyComposeFont(fontKey) {
  ...
  messageTextEl.classList.remove(
    ...
    "font-bn-siliguri", "font-bn-baloo", "font-bn-notoserif", "font-bn-sans"
  );
...
function applyComposeBg(bgKey) {
  ...
  letterFormCard.classList.remove(
    ...
    "letter-bg-bn-paper", "letter-bg-bn-rose", "letter-bg-bn-mint", "letter-bg-bn-sky"
  );
