...
### 5. Keep Firestore TTL disabled

Messages are deleted only when the recipient deletes them from the inbox or
the sender uses their unsend link. Do not create a Firestore TTL policy for the
`messages` collection.

If you already enabled TTL before, delete/disable the TTL policy for:
...
Messages do not auto-delete. Recipients can delete messages from the inbox, and
senders can unsend a specific message with their private delete link.
...
- ✅ Manual delete by recipient or sender unsend link
