...
//    → "Web Push certificates" → Generate key pair → copy it
// ============================================================
...
let allMessageDocs = [];
let visibleCount = 5;
// Track IDs we've already seen so the first snapshot doesn't fire
// notifications for letters that already existed before the page loaded.
...
// ─── FCM: permission + token storage + foreground handler ────
async function initPushNotifications(uid) {
  // Notifications not supported (e.g. Firefox private mode, older iOS)
  if (!("Notification" in window) || !("serviceWorker" in navigator)) return;

  try {
    // 1. Ask permission (browser shows native prompt)
    const permission = await Notification.requestPermission();
    if (permission !== "granted") return; // user declined — silent exit

    // 2. Register the FCM service worker
    ...
    if (!token) return; // FCM couldn't produce a token

    // 4. Persist the token in the user's Firestore doc so your
    //    backend (Cloud Function / server) can send targeted pushes.
    ...
    // 5. Handle FOREGROUND messages (tab is open & visible).
    //    When the tab is hidden/closed the service worker handles it.
    onMessage(messaging, (payload) => {
      const title = payload.notification?.title || "💌 New letter arrived!";
      const body  = payload.notification?.body  || "";
      showForegroundNotification(title, body);
    });

  } catch (err) {
    // Non-fatal — push is enhancement only; inbox still works fine
    console.warn("Push notification setup failed:", err);
  }
}

// Show a browser Notification when the tab is in the foreground.
// The Service Worker cannot show notifications while the page is focused
// (Chrome suppresses them), so we use the Notification API directly.
...
function loadMessages(uid) {
  ...
  unsubscribeMessages = onSnapshot(q, (snapshot) => {
    ...
    // ── Detect newly added letters & show notification ────────
    if (seenMessageIds === null) {
      // First snapshot: record existing IDs, no notification yet
      seenMessageIds = new Set(snapshot.docs.map(d => d.id));
    } else {
      snapshot.docChanges().forEach((change) => {
        if (change.type === "added" && !seenMessageIds.has(change.doc.id)) {
          ...
          showForegroundNotification("💌 New letter arrived!", preview);
        }
      });
    }
    // ─────────────────────────────────────────────────────────
